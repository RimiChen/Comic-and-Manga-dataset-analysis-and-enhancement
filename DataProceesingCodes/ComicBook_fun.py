...
def test_fun():
    print("test whether imported success!")
    
def comic_annotaiton_processing(annotation_file_path, target_folder_path, book_index):
    ...
    root_path = target_folder_path[:-1]
    root_path = root_path.replace(str(book_index), "") 
    print(root_path)
    ...
    json_file = open(target_folder_path+"/"+str(newComicBook.bookName)+"_characters.json", "w")
    # magic happens here to make it pretty-printed
    json_file.write(json.dumps(newComicBook.characters, indent=4))
    json_file.close()   

    json_file = open(root_path+str(book_index)+"_characters.json", "w")
    # magic happens here to make it pretty-printed
    json_file.write(json.dumps(newComicBook.characters, indent=4))
    json_file.close()   
        
    json_file = open(target_folder_path+"/"+str(newComicBook.bookName)+"_pages.json", "w")
    # magic happens here to make it pretty-printed
    json_file.write(json.dumps(newComicBook.pageList, indent=4))
    json_file.close()   

    json_file = open(root_path+str(book_index)+"_pages.json", "w")
    # magic happens here to make it pretty-printed
    json_file.write(json.dumps(newComicBook.pageList, indent=4))
    json_file.close()   
