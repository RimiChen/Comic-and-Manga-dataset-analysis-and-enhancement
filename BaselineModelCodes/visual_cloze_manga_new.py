...
import theano, cPickle, lasagne, random, csv, gzip, time, argparse, sys, os
import numpy as np
import h5py as h5
import theano.tensor as T   
from collections import Counter
sys.path.append(os.path.join(os.path.dirname(sys.path[0]),'preprocess'))
from visual_cloze_minibatching import *
...
def build_image_feature_network(d_word, d_hidden, lr, eps=1e-6):
...
## -----------------------------
    ...
    in_transitions = T.tensor3(name='transitions')
    in_panel_info = T.tensor3(name='panel_info')
    in_layout_info = T.tensor3(name='layout_info')

            # h_transitions = f.create_dataset(fold+'/transitions',
            #         shape=(d[fold]['n'], self.max_panels), dtype=np.double)
            # h_panel_info = f.create_dataset(fold+'/panel_info',
            #         shape=(d[fold]['n'], self.max_panels), dtype=np.double)
            # h_layout_info = f.create_dataset(fold+'/layout_info',
            #         shape=(d[fold]['n'], self.max_panels), dtype=np.double)
    
    ## R
    in_feature = T.tensor3(name="image_feature")
...
#### -----------------------------
    ...
    l_transitions = lasagne.layers.InputLayer(shape=(None, 3, 4096), input_var=in_transitions)
    l_panel_info = lasagne.layers.InputLayer(shape=(None, 3, 4096), input_var=in_panel_info)
    l_layout_info = lasagne.layers.InputLayer(shape=(None, 3, 4096), input_var=in_layout_info)


    
    l_feature = lasagne.layers.InputLayer(shape=(None, 3, 4096), input_var=in_feature)
#### -----------------------------
...
###### ----------------------------
    ...
    l_context_concat = MyConcatLayer([l_context_panel_reps, l_context_fc7, l_transitions, l_panel_info, l_layout_info], axis=-1)
...
if __name__ == '__main__':
    ...
    ##### need to change the dataset
    parser.add_argument('-data', default=DATA_PATH+'manga_random_13.h5')
    parser.add_argument('-vocab', default=DATA_PATH+'comics_vocab.p')
    parser.add_argument('-model', default='image_text', 
        help='image_text, image_only, text_only, or image_feature')
    ##### cheange to imagenet version
    parser.add_argument('-vgg_feats', default=DATA_PATH+'manga_vgg_panel_info.h5')
    # # with text score    
    # parser.add_argument('-vgg_feats', default=DATA_PATH+'manga_vgg_panel_with_textdense.h5')    
    # # with layout complexity score    
    # parser.add_argument('-vgg_feats', default=DATA_PATH+'manga_vgg_panel_with_layout.h5')
    # # with transition guessing    
    # parser.add_argument('-vgg_feats', default=DATA_PATH+'manga_vgg_panel_with_transitions.h5')    
    # # with information score guessing
    # parser.add_argument('-vgg_feats', default=DATA_PATH+'manga_vgg_panel_info_score.h5')    
    ...
    build_dict = {'image_text': build_image_text_network, 
        ...
        'image_feature':build_image_feature_network}
