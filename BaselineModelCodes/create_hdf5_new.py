...
class Create_hdf5:
    ...
    def __init__(self, panels_path, ocr_file, ad_path, dims, max_panels, max_boxes, max_words, max_vocab_size, h5path):
        ...
        self.dump_vocabulary(DATA_PATH+'manga_vocab.p')
    ...
    def compute_fold_starts(self):
        ...
        ####### modify here to decide how the test and dev splited
        ...
        for row in f:
            ...
            #print("current_book = "+str(curr_book)+", current_page = "+str(curr_page))
    ...
    def create_img_hdf5(self):
        ...
        for fold in d.keys(): # loop through train, test, dev
            ...
            #### for each page load the layout parameter
            layout_parameter = random.uniform(0, 1)
            ...
            for i, key in enumerate(d[fold]['keys']):
               ...
               ##### modify here to print current progress
                print("manga_"+str(comic_no)+", page_"+str(page_no)+", panel_"+str(panel_no))
                ...
                if len(panel_contents) == 1 and panel_contents[0][0] == None:
                    ...
                    ###
                    # h_features[curr_page_idx, panel_no] = 1
                    ### style features that influenced by textbox
                    h_image_info[curr_page_idx, panel_no] = 1
                    ...
                    #h_features[curr_page_idx, panel_no] = 1
                    ### style features that influenced by textbox
                    h_panel_info[curr_page_idx, panel_no] = -1

                #### put the transition data here
                h_transitions[curr_page_idx, panel_no] = random.uniform(0, 1)
                h_layout_info[curr_page_idx, panel_no] = layout_parameter
...
def main():
    ...
    panels_path = DATA_PATH+'raw_panel_random_test/' 
    ##### change OCR files
    ocr_file = DATA_PATH+'MANGA_ocr_file_13.csv' 
    ad_path = DATA_PATH+'predadpages_manga.txt' # these pages have ads according to ad filter
    ##### change dataset
    h5path = DATA_PATH+'manga_random_13.h5' # dumping h5 file at this location
