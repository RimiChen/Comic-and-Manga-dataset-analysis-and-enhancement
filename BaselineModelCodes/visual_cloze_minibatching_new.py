...
def generate_minibatches_from_megabatch(data, mb_start, mb_end, batch_size=64, 
    ...
    #### layout_info, panel_info, transitions,
...
if __name__ == '__main__':
    vdict, rvdict = pickle.load(open(DATA_PATH+'comics_vocab.p', 'rb'))
    #comics_data = h5.File('manga_data/manga.h5', 'r')
    
    #### need to change the dataset
    comics_data = h5.File(DATA_PATH+'manga_random_13.h5', 'r')
    
    ##### cheange to imagenet version
    # comics_fc7 = h5.File(DATA_PATH+'vgg_features.h5')
    comics_fc7 = h5.File(DATA_PATH+'manga_vgg_panel_info.h5')
    ...
    for difficulty in ['easy', 'hard']:
        for fold in ['dev', 'test']:
            ...
            #### Add new features
            # layout_info = fold_data["layout_info"]
            # panel_info = fold_data["panel_info"]
            # transitions = fold_data["transitions"]
            # data = (img_mask, book_ids, page_ids, bboxes, bbox_mask, words, \
            #     word_mask, comics_fc7[fold]['vgg_features'])

            # data = (img_mask, book_ids, page_ids, bboxes, bbox_mask, words, \
            #     word_mask, comics_fc7['vgg_features'])
